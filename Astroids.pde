class Astroids extends Floater {
  ...
  public Astroids() {
    ...
    xCorners[0] = -11;
    yCorners[0] = -4 + (int)(Math.random()*9);
    xCorners[1] = 7 + (int)(Math.random()*2);
    yCorners[1] = -8;
    xCorners[2] = 3 - (int)(Math.random()*8);
    yCorners[2] = 0 + (int)(Math.random()*3);
    xCorners[3] = 6 + (int)(Math.random()*1);
    yCorners[3] = 10 + (int)(Math.random()*6);
    xCorners[4] = -12 - (int)(Math.random()*2);
    yCorners[4] = 7 - (int)(Math.random()*1);
    xCorners[5] = -3 + (int)(Math.random()*1);
    yCorners[5] = 7 + (int)(Math.random()*9);
    myColor = color(255);
    myXspeed = 1.0;
